...
my_api_key = "AIzaSyDYV1h4lUnR6H8eVYS4eXvtMHKwEarr9Mg"
...
model = genai.GenerativeModel('models/gemini-1.5-flash-latest',
    generation_config=genai.GenerationConfig(
        ...
        top_p = 0.95,
        top_k = 64,
        max_output_tokens=70, # 100 tokens correspond to roughly 60-80 words.
        temperature = 0.9,
    ))
...
# convert "text" to audio file, save to disk, play back, and delete
def speak_text(text):
    global slang, rled
    ...
    try:
        ...
        if os.path.exists(audio_file):
            ...
            print(f"Audio file {audio_file} deleted successfully.")
        else:
            print(f"Audio file {audio_file} not found for deletion.")

    except KeyboardInterrupt:
        mixer.music.stop()
        if os.path.exists(audio_file):
            os.remove(audio_file)
            print(f"Audio file {audio_file} deleted successfully due to interrupt.")
        rled.off() 
        ...
        if os.path.exists(audio_file):
            os.remove(audio_file)
            print(f"Audio file {audio_file} deleted successfully after error.")
        rled.off()
